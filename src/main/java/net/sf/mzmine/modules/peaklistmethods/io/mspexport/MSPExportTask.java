...
public class MSPExportTask extends AbstractTask {
    ...
    private final boolean savePrecursor;
    ...
    MSPExportTask(ParameterSet parameters) {
        ...
        this.savePrecursor = parameters.getParameter(MSPExportParameters.SAVE_PRECURSOR)
                .getValue();
    }
...
//                if (name != null) writer.write("Name: " + name + newLine);
            ...
            if (savePrecursor) {
                String precursor = Double.toString(row.getAverageMZ());
                if (precursor != null)
                    writer.write("PrecursorMz: " + precursor + newLine);
            }
